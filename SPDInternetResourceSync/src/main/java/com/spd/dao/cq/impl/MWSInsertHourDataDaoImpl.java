 ...
 *
 */
public class MWSInsertHourDataDaoImpl extends BaseDao {
...
//	}
	...
	public HashMap<String, Object> getExistMCI(String datetime) {
		//mysql
		String query = "select Station_Id_C, date_format(datetime, '%Y-%m-%d') as datetime from t_mwshourrain2 where datetime = '" + datetime + "'";		
...
//		String query = "select * from 小时降水 where 时次 >= '" + startDateTime + "' and 时次 <= '" + endDateTime + "'";		
		String query = "select * from 小时降水 where 时次 = '20140901070000'";		
		List list = query(getConn(), query, null);
		...
		for(int i = 0; i < list.size(); i++) {
			...
			if((R1 != null && R1.floatValue() !=0) || (R3 != null && R3.floatValue() !=0) || 
				...
				dataMap.put("datetime", tempMap.get("日期") + " 00:00:00");
