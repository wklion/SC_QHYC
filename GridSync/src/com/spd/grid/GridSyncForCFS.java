...
//			ds = m_workspace.CreateDatasource(m_strConnectionInfo);
	...
	public ArrayList<File> getFiles(Date date) {
		...
		String filter = "(.*)01.date(.*).daily.grb2.nc";
	...
	public void sync(ArrayList<File> files) {
		...
		for(int i=0; i<files.size(); i++)
		{
			...
			int hourSpanInterval = 6;
			String strDateTime = strs[2];
			...
			for(int k=0; k<dsGrib.GetDatasetCount(); k++){
				...
				if(!dsGrib.GetDataset(k).GetType().equals("Raster"))
					continue;
				if(hourSpan>24*35){//只要35天的数据
					break;
				}
				DatasetRaster dr = (DatasetRaster)dsGrib.GetDataset(k);
				String strForecastDate = strYear+strMonth+strDay+strHour;
				String desDatasetName = String.format("t_%s_%s_%s00_p_%s_%03d_%s", m_strType, m_strElement, strForecastDate, strForecastDate, hourSpan, m_strLevel);
				
				String desDatasetNameStatistics = "";
				if(bStatistics)
				{
					int currentStatisticsHourSpan = (int)Math.ceil((double)(hourSpan)/(double)(m_nStatisticsHourSpan))*m_nStatisticsHourSpan;
					desDatasetNameStatistics = String.format("t_%s_%s_%s00_p_%s_%03d_%s", m_strType, m_strElement, strForecastDate, strForecastDate, currentStatisticsHourSpan, m_strLevel);
...
//					            if(detaX > m_dResolutionX)
					          ...
					          //赋值到标准（产品）网格
					            ...
					            if(dtResample != null){
									...
									if(dgTarget != null){
										...
										Boolean bTemp =m_strElement.equals("2t");
			    						...
			    						for (int kk = 0; kk<rows; kk++)
							            {
			    							...
			    							for(int ll=0; ll<cols; ll++){
			    								if(bTemp){
			    									sl.SetValue(ll, slResample.GetValue(ll+offsetX)-273.15); //这里是变温，用的K值
			    								}
			    								else{
			    									sl.SetValue(ll, slResample.GetValue(ll+offsetX)*3600*6); //降水量p=r*3600*6
			    								}
			    							}
	...
	public static void main(String[] args) {
		//args = new String[]{"cfs", "r24", "1000", "C:/Users/wk/Desktop/Test/Prec", "false", "24", "sum"};
		args = new String[]{"cfs", "2t", "1000", "E:/SC/Data/CFS/Temp", "false", "24", "max"};
		//args = new String[]{"cfs", "r24", "1000", "E:/Data/CFS/Cfs_Prec", "false", "24", "sum"};
		...
		GridSyncForCFS gridSync = new GridSyncForCFS();
