...
;(function ( $, window, document, undefined ) {
  ...
  $(document).on('click', '[data-buttons=dropdown]', function(e) {
	if(e.target.className == "button button-rounded"){
		//alert(e.target.innerText);
        var str = e.target.innerText;
        var strSymbol = str.substr(0, 1);
        var val = Number(str.substr(1));
        if(strSymbol == "+")
            val *= 1;
        else if(strSymbol == "-")
            val *= -1;
        GDYB.GridProductClass.updateGrid(val, 1);
        GDYB.FilterTool.updateGrid(val, 1);
	}
	else{
		var $dropdown = $(e.currentTarget);
		$dropdown.menuButton();
        if(e.target.localName == "a"){
            e.currentTarget.firstChild.firstChild.data = e.target.innerText;
            ...
            {
                val *= 1;
                GDYB.GridProductClass.addValue = val;
            }
            else if(strSymbol == "-")
            {
                val *= -1;
                GDYB.GridProductClass.subValue = val;
            }
        }
	}    
  });
